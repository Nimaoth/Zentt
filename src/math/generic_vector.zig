    ...
    return struct {
        ...
        usingnamespace switch (dimensions) {
            ...
            3 => extern struct {
                ...
                pub fn toImgui4(self: Self, w: T) imgui.Vec2 {
                    return imgui.Vec4{ .x = self.x(), .y = self.y(), .z = self.z(), .w = w };
                }
            },
            4 => extern struct {
                /// Construct new vector.
                ...
                pub fn toImgui(self: Self) imgui.Vec4 {
                    return imgui.Vec4{ .x = self.x(), .y = self.y(), .z = self.z(), .w = self.w() };
                }
            },
            else => {},
        };
        ...
        pub fn get(val: anytype, comptime component: anytype) T {
            const ValType = @TypeOf(val);
            const component_name = comptime std.meta.tagName(component);
            if (@hasField(ValType, component_name)) {
                // const FieldType = @TypeOf(@field(val, component_name));
                return @field(val, component_name);
            } else {
                return 0;
            }
        }
        ...
        pub fn from(val: anytype) Self {
            return switch (dimensions) {
                2 => Self.new(get(val, .x), get(val, .y)),
                3 => Self.new(get(val, .x), get(val, .y), get(val, .z)),
                4 => Self.new(get(val, .x), get(val, .y), get(val, .z), get(val, .w)),
                else => unreachable,
            };
        }
        ...
        /// Multiplication betwen two given vector.
